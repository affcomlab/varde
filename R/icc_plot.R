#' @export plot.varde_icc_lme
#' @export
#' @importFrom graphics plot
plot.varde_icc_lme<- function(x, ...) {
...
#' @export plot.varde_icc_brms
#' @export
#' @importFrom graphics plot
plot.varde_icc_brms <- function(x, ...){
  plot_varde_brms(x, ...)
}
...
plot_varde_brms <- function(x, parameters = NULL,
                            font_size = 10,
...
plot_varde_lme <- function(x, order = NULL, type = "point",
                           parameters = NULL,
                           font_size = 10,
                           fill_color = "lightblue",
                           panel_spacing = 2,
  ...
  if (type == "point") {
    ...
    #variances
    dat_var <- x$vars_summary %>%
      dplyr::mutate(name = forcats::fct_relevel(component, ord_var))
    ...
    #iccs
    dat_icc <- x$iccs_summary %>%
      dplyr::mutate(name = forcats::fct_relevel(term, ord_icc))
